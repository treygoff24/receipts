...
pub fn run(_global: &GlobalArgs) -> Result<CommandSuccess<Capabilities>, ReceiptsError> {
    ...
    let data = Capabilities {
        ...
        exit_codes: ExitCodes {
            ...
            config: "config",
            network: "network; zero-claim total provider failure; retry with backoff",
            upstream: "upstream; zero-claim total provider failure; retry once",
            rate_limit: "rate-limit; zero-claim total provider failure; back off and retry",
